...
const PRESETS = [
  ...
  {
    id: 7,
    name: "FotografÌa Urbana Street",
    subtitle: "EnergÌa callejera",
    free: false,
    promptBlock: "Urban street photography...",
  },
  {
    id: 8,
    name: "EnsueÒo Vintage 70s",
  ...
  {
    id: 12,
    name: "AcciÛn Deportiva Congelado",
    subtitle: "Movimiento nÌtido",
    free: false,
  ...
  {
    id: 14,
    name: "FantasÌa Surrealista EtÈreo",
    subtitle: "OnÌrico y de otro mundo",
    free: false,
...
const SCENARIOS = [
  ...
  {
    id: 6,
    name: "Bosque Niebla AtmosfÈrico",
...
const SUBSCRIPTION_PLANS = [
  ...
  },
  {
    ...
    features: [
      "60 crÈditos/mes",
      "3 prompts personalizados (24‚Äì48h)",
...
const GeminiAssistantView = ({ onCopy, isPro }) => {
  ...
  const handleGenerate = async (e) => {
    ...
    if (profile?.credits <= 0) {
      setResponse(
        "No tienes crÈditos disponibles. Compra crÈditos o suscrÌbete."
      );
          ...
          >
            ...
            >
              {profile.credits === 0
                ...
                : `‚ö†Ô∏è Te quedan ${profile.credits} crÈdito${
                    profile.credits === 1 ? "" : "s"
                  }.`}
            </p>
            {profile.plan === "free" && (
          ...
          <div className="mt-6">
            ...
            <div className="bg-black/40 border border-[color:var(--border)] rounded-lg p-4">
              <pre className="text-gray-300 whitespace-pre-wrap font-sans text-sm">
                {response || "AquÌ aparecer· el prompt generado..."}
              </pre>
              {response && (
                    ...
                    onClick={() => {
                      ...
                      if (window.App_showToast)
                        window.App_showToast(
                          "Prompt copiado. Abriendo Gemini‚Ä¶"
                        );
                    }}
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-xl font-bold">
          Historial de Prompts
          {profile.plan === "free" && (
            <span className="text-sm text-gray-400 ml-2">(˙ltimos 3)</span>
          )}
        </h3>
...
function AppContent() {
  const { user, profile, signOut, refreshProfile } = useAuth(); // ? A—ADIR refreshProfile
  ...
  // ? NUEVO: Detectar retorno de Stripe (SIN RECARGAR)
  useEffect(() => {
    const handlePaymentReturn = async () => {
      ...
      if (payment === "success" && sessionId) {
        console.log("? Pago exitoso detectado - Session:", sessionId);

        // Limpiar URL inmediatamente
        window.history.replaceState({}, "", "/");

        // Mostrar overlay de Èxito
        setShowPaymentSuccess(true);

        // ? ESPERAR A QUE REALTIME ACTUALICE (m·x 10 segundos)
        console.log("? Esperando actualizaciÛn en tiempo real...");

        // Timeout de seguridad: si en 10 segundos no se actualiza, cerrar modal
        const timeoutId = setTimeout(() => {
          console.log("? Timeout alcanzado - Cerrando modal");
          setShowPaymentSuccess(false);
          setView("profile"); // Ir al perfil para ver los crÈditos
        }, 10000);

        // Guardar el timeout ID para poder cancelarlo si se actualiza antes
        window.paymentSuccessTimeout = timeoutId;
  ...
  // ? NUEVO: Detectar cuando el perfil se actualiza (por Realtime)
  useEffect(() => {
    if (showPaymentSuccess && profile?.credits > 0) {
      console.log("?? Perfil actualizado por Realtime:", profile);

      // Cancelar timeout si existe
      if (window.paymentSuccessTimeout) {
        clearTimeout(window.paymentSuccessTimeout);
        delete window.paymentSuccessTimeout;
      }

      // Esperar 2 segundos para que el usuario vea el mensaje
      setTimeout(() => {
        setShowPaymentSuccess(false);
        setView("profile"); // Ir al perfil
      }, 2000);
    }
  }, [profile, showPaymentSuccess]);
  ...
  const handlePlanSelection = async (planId) => {
    ...
    // ? OPCI”N A: Mostrar modal de confirmaciÛn (paso extra)
    setSelectedPlan(planId);
    setShowCheckout(true);

    // ? OPCI”N B: Redirigir directamente (tu cÛdigo actual)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900">
              ...
              <div className="max-w-7xl mx-auto">
                <div className="text-center mb-12">
                  <h2 className="text-4xl font-bold mb-4">
                    GalerÌa de Prompts Profesionales
                  </h2>
                  <p className="text-muted text-lg">
                    Explora nuestra colecciÛn de prompts optimizados
                  </p>
                </div>
                  ...
                  {[
                    ...
                    >
                      ...
                      <p className="text-sm text-muted">
                        Preset optimizado para fotografÌa {preset.toLowerCase()}
                      </p>
                    </div>
                  ))}
                </div>
            ...
            <section className="py-12 px-4">
              <div className="max-w-7xl mx-auto text-center">
                <h3 className="text-2xl font-heading font-semibold mb-3">
                  GuÌa para crear PROMPTS de retratos profesional{" "}
                  <span className="text-[color:var(--primary)]">GRATIS</span>
                </h3>
                <p className="text-lg text-muted mb-6">
                  Descarga nuestra guÌa en pdf
                </p>
                <a
      ...
      {/* Payment Success Overlay - ACTUALIZADO */}
      {showPaymentSuccess && (
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-50">
          <div className="max-w-md w-full bg-[color:var(--surface)] rounded-2xl p-8 border border-[color:var(--border)] text-center mx-4">
            ...
            {/* AnimaciÛn de carga mientras esperamos Realtime */}
            <div className="space-y-3">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[color:var(--primary)] mx-auto"></div>
              <p className="text-sm text-[color:var(--primary)] font-semibold animate-pulse">
                Actualizando tus crÈditos...
              </p>
            ...
            <div className="mt-6 w-full bg-white/10 rounded-full h-2 overflow-hidden">
              <div
                className="bg-[color:var(--primary)] h-full rounded-full animate-pulse"
                style={{ width: "60%" }}
              ></div>
            </div>
          </div>
