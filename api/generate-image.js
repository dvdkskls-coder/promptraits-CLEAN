...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Aceptamos un array de imágenes o nada
    ...
    // Si hay imágenes, las añadimos (ahora es opcional)
    if (faceImages && Array.isArray(faceImages)) {
        faceImages.forEach(img => {
            if (img && img.base64) {
                ...
                parts.push({
                    inlineData: { data: cleanBase64, mimeType: img.mimeType || 'image/jpeg' }
                });
            }
        });
    }
    ...
    const responsePart = result.response.candidates?.[0]?.content?.parts?.find(p => p.inlineData);
    
    if (!responsePart) throw new Error('No se pudo generar la imagen.');
